 ...
 * @since 1.0.0
 */
class MapaTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_mantenimiento.mapa';
		parent::__construct('#__mantenimiento_mapa', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!$user->authorise('core.admin', 'com_mantenimiento.mapa.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_mantenimiento/access.xml',
				"/access/section[@name='mapa']/"
			);
			$default_actions = Access::getAssetRules('com_mantenimiento.mapa.' . $array['id'])->getData();
			$array_jaccess   = array();
