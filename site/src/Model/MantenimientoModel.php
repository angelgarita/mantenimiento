 ...
 * @since  1.0.0
 */
class MantenimientoModel extends ItemModel
{
	public $_item;
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_mantenimiento')) && (!$user->authorise('core.edit', 'com_mantenimiento')))
		{
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_MANTENIMIENTO_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_MANTENIMIENTO_ITEM_NOT_LOADED'), 404);
			}
		}
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('mantenimiento');
		}
